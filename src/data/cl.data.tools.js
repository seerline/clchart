...
function _getExrightPara(rightdata) {
  let exright_gs = 1000; // 送股数
  let exright_pg = 0; // 配股数
  let exright_px = 0; // 利息
  if (rightdata[FIELD_RIGHT.accrual]) exright_px = rightdata[FIELD_RIGHT.accrual] / 10;
  if (rightdata[FIELD_RIGHT.sendstock] > 0 || rightdata[FIELD_RIGHT.allotstock] > 0) {
    exright_gs = 1000 + rightdata[FIELD_RIGHT.sendstock] / 10 + rightdata[FIELD_RIGHT.allotstock] / 10;
    exright_pg = -rightdata[FIELD_RIGHT.allotprice] * rightdata[FIELD_RIGHT.allotstock] / 10000;
  }
  return {
    gs: exright_gs,
    pg: exright_pg,
...
export function checkDay5(source, tradedate) {
  ...
  const lastDate = getDate(source[source.length - 1][FIELD_TICK.time]);
  // 判断是否已经有收盘数据了
  let maxDays = 5;
